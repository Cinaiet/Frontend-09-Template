...
function layout(element) {
  ...
  if(style.flexDirection == 'row') {
    ...
    crossStart = 'top'
    crossEnd = 'bottom'
  } else if(style.flexDirection == 'flex-reverse') {
    mainSize = 'width'
    mainStart = 'right'
    mainEnd = 'left'
    mainSign = -1
  ...
  for(var i = 0; i < list.length; i++) {
    ...
    if(mainSpace < 0) {
      var scale = style[mainSign] / (style[mainSize] - mainSpace)
      ...
      for(var i = 0; i < list.length; i++) {
        ...
        itemStyle[mainSize] = itemStyle[mainSign] * scale
    ...
    flexLines.forEach( function(item) {
      ...
      for(var i = 0; i < item.length; i++) {
        ...
        if(align == 'stretch') {
          itemStyle[crossStart] = crossBase
          itemStyle[crossEnd] = itemStyle[crossStart] + crossSign * (itemStyle[crossSize] != null && itemStyle[crossSize] !== (void 0) ? 
...
function getStyle(element) {
  ...
  if(Object.keys(element.computeStyle).length) {
    for(let prop in element.computeStyle) {
      const val = element.computeStyle.value  
